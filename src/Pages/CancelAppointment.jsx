...
function CancelAppointmentModal() {
  ...
  const { selectedDoctor, selectedDate, selectedTime, setSelectedDoctor } = useDoctor();
    ...
    <div className="fixed inset-0 bg-black/35 flex justify-center items-center z-[999]">
      <div className="w-[470px] bg-[#f0f0f0] rounded-lg p-8 font-serif">
        ...
        <p className="text-lg-gray-700 ">
          ...
          <strong className="font-bold">{selectedDoctor?.name}</strong> on{" "}
          <strong className="font-bold">
            {selectedDate?.month} {selectedDate?.date}
          </strong>{" "}
          at <strong className="font-bold">{selectedTime}</strong>?
        </p>
